   ...
   <pattern>
      ...
      <rule context="tradeItem">
		  <assert test="if((targetMarket/targetMarketCountryCode =  ('249'(: France :) , '250'(: France :) )) 
    					...
    					and (tradeItemInformation/extension/*:packagingInformationModule/packaging/platformTypeCode = '42'))
    					...
    					then   ((number($node/depth) ge 1100) and (number($node/depth) le 2200)
            			and    (number($node/width) ge 1100) and (number($node/width) le 2200))                			
            			else  (: measurementUnitCode = 'IN' :)    
            			     if (($node/depth/@measurementUnitCode = 'IN') and  ($node/width/@measurementUnitCode = 'IN'))              			    
            			then   ((number($node/depth) ge 43.31) and (number($node/depth) le 86.62)
            			and    (number($node/width) ge 43.31) and (number($node/width) le 86.62))                   			 
            				else true()  
            					else true())) 												
        							
				          				 else true()">				          		 
				          		 		
							      		 		<errorMessage>If targetMarketCountryCode equals ('249' (France) or '250' (France)) and tradeItemUnitDescriptorCode equals 'PALLET' and platformTypeCode equals  '42',
							      		 		              then TradeItemMeasurements/depth shall bebetween and including ('1100 MMT' or '43.31 IN') and ('2200 MMT' or '86.62 IN')
							      		 		              and TradeItemMeasurements/width shall be between and including ('1100 MMT' or '43.31 IN') and ('2200 MMT' or '86.62 IN').
	                                            </errorMessage>
