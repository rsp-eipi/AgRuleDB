...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
   ...
   <pattern>
      ...
      <doc:description>If targetMarketCountryCode equals ('056' (: Belgium :), '442' (: Luxembourg :), '528' (: Netherlands :), '276' (: Germany :), '208' (: Denmark :), 
                       ...
                       and one instance of nutrientTypeCode equals 'STARCH' and quantityContained is used 
                       and another instance of nutrientTypeCode equals 'CHOAVL' and quantityContained is used, 
                       then quantityContained for nutrientTypeCode 'STARCH' SHALL be less than or equal to quantityContained for nutrientTypeCode 'CHOAVL'.
      </doc:description>
      ...
      <rule context="tradeItem">
		  <assert test="if (targetMarket/targetMarketCountryCode =  ('056', '442', '528', '276', '208', '203', '246', '826', '380', '250', '040'))
		                then (every $node in (tradeItemInformation/extension/*:nutritionalInformationModule/nutrientHeader[exists(./nutrientDetail/nutrientTypeCode[.='STARCH'])
		                and exists(./nutrientDetail/nutrientTypeCode[.='CHOAVL'])]) 
		                satisfies(every $node1 in ($node/nutrientDetail[nutrientTypeCode[.='STARCH']]/quantityContained)
		                satisfies (every $node2 in ($node/nutrientDetail[nutrientTypeCode[.='CHOAVL']]/quantityContained) satisfies (number($node1) &lt;= number($node2))))) 
							      		 		...
							      		 		<errorMessage>Quantity of starch ('STARCH') is greater than the quantity of carbohydrates ('CHOAVL').</errorMessage>
