   ...
   <pattern>
      ...
      <rule context="tradeItem">
		  <assert test="if((targetMarket/targetMarketCountryCode =  ('249'(: France :) , '250'(: France :) )) 
    					...
    					and (tradeItemInformation/extension/*:packagingInformationModule/packaging/platformTypeCode = '41'))
    					...
    					then   ((number($node/depth) ge 1067) and (number($node/depth) le 2134)
            			and    (number($node/width) ge 1067) and (number($node/width) le 2134))                			
            			else  (: measurementUnitCode = 'IN' :)    
            			     if (($node/depth/@measurementUnitCode = 'IN') and  ($node/width/@measurementUnitCode = 'IN'))              			    
            			then   ((number($node/depth) ge 42.01) and (number($node/depth) le 84.02)
            			and    (number($node/width) ge 42.01) and (number($node/width) le 84.02))                   			 
            				else true()  
            					else true())) 												
        							
				          				 else true()">				          		 
				          		 		
							      		 		<errorMessage>If targetMarketCountryCode equals ('249' (France) or '250' (France)) and  tradeItemUnitDescriptorCode equals 'PALLET' and platformTypeCode equals  '41',
							      		 		              then TradeItemMeasurements/depth shall be between and including ('1067 MMT' or '42.01 IN') and ('2134 MMT' or '84.02 IN')
							      		 		              and TradeItemMeasurements/width shall be between and including ('1067 MMT' or '42.01 IN') and ('2134 MMT' or '84.02 IN').
	                                            </errorMessage>
