   ...
   <pattern>
      ...
      <rule context="tradeItem">
          <assert test="if((targetMarket/targetMarketCountryCode =  ('249'(: France :) , '250'(: France :) )) 
        					...
        					and (tradeItemInformation/extension/*:packagingInformationModule/packaging/platformTypeCode = '10'))
        					...
        					then   ((number($node/depth) ge 800) and (number($node/depth) le 1600)
                			...
                			then   ((number($node/depth) ge 31.50) and (number($node/depth) le 63)
                			and    (number($node/width) ge 23.62) and (number($node/width) le 47.24))                   			 
                				else true() (: incomparable dimensions :)	
                					else true()))
				          				 else true()">
				          		 
				          		 		
						      		 		<errorMessage>If targetMarketCountryCode equals ('249' (France) or '250' (France))  and tradeItemUnitDescriptorCode equals 'PALLET' and platformTypeCode equals '10', then depth shall be between and including  ('800 MMT' or '31.50 IN')  and ('1600 MMT' or '63 IN') and width shall be between and including (' '600 MMT' or '23.62 IN') and ('1200 MMT' or '47.24 IN').</errorMessage>
