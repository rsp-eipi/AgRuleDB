   ...
   <pattern>
      ...
      <rule context="tradeItem">
		  <assert test="if((targetMarket/targetMarketCountryCode =  ('249'(: France :) , '250'(: France :) )) 
    					...
    					and (tradeItemInformation/extension/*:packagingInformationModule/packaging/platformTypeCode = '13'))
    					...
    					then   ((number($node/depth) ge 600) and (number($node/depth) le 1200)
            			and    (number($node/width) ge 400) and (number($node/width) le 800))                			
            			else  (: measurementUnitCode = 'IN' :)    
            			     if (($node/depth/@measurementUnitCode = 'IN') and  ($node/width/@measurementUnitCode = 'IN'))              			    
            			then   ((number($node/depth) ge 23.62) and (number($node/depth) le 47.24)
            			and    (number($node/width) ge 15.75) and (number($node/width) le 31.5))                   			 
            				else true()  
            					else true())) 												
        							
				          				 else true()">				          		 
				          		 		
							      		 		<errorMessage>If targetMarketCountryCode equals ('249' (France) or '250' (France)) and  tradeItemUnitDescriptorCode equals 'PALLET' and platformTypeCode equals  '13',
							      		 		              then TradeItemMeasurements/depth shall be between and including ('600 MMT' or '23.62 IN') and ('1200 MMT' or '47.24 IN'), 
							      		 		              and TradeItemMeasurements/width shall be between and including ('400 MMT' or '15.75 IN') and ('800 MMT' or '31.5 IN').
	                                            </errorMessage>
