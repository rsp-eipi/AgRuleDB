...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
   ...
   <pattern>
      <title>Rule 1744</title>      
      <doc:description>If targetMarketCountryCode equals ('203' (Czech Republic), 246' (Finland), or '703' (Slovakia)) and ((gpcCategoryCode equals a brick value from segment '50000000' excluding brick values from family '50210000') or (gpcCategoryCode equals '10000467', '10000468' or '10000651')) and isTradeItemAConsumerUnit is equal to 'true' and (preliminaryItemStatusCode is not used or equals 'FINAL’), then regulatedProductName SHALL be used.</doc:description>                      
      <doc:avenant>Modification 3.1.17 Le 16/03/2022</doc:avenant>       
      <doc:avenant1>WR-22-382: Added statements into the structured rule, error message and example of data that will pass.</doc:avenant1>
      <doc:avenant2>Modif. 3.1.25 La règle s'applique également sur le marché cible Slovaquie</doc:avenant2>       
      <doc:attribute1>targetMarketCountryCode,gpcCategoryCode,isTradeItemAConsumerUnit,preliminaryItemStatusCode</doc:attribute1>   
      <doc:attribute2>regulatedProductName</doc:attribute2>     
      <rule context="tradeItem">
     		   <assert test="if ((targetMarket/targetMarketCountryCode =  ('203', '246', '703'))	
     		            ...
     		            and ((not(exists(preliminaryItemStatusCode) or (preliminaryItemStatusCode = 'FINAL' )))))
		                ...
		                and  (every $node in (tradeItemInformation/extension/*:tradeItemDescriptionModule/tradeItemDescriptionInformation/regulatedProductName)
         				  satisfies  ($node !='' ) )                  
				          		 				...
				          		 				<gpcCategoryCode><xsl:value-of select="gdsnTradeItemClassification/gpcCategoryCode"/></gpcCategoryCode>	
				          		 				<regulatedProductName><xsl:value-of select="tradeItemInformation/extension/*:tradeItemDescriptionModule/tradeItemDescriptionInformation/regulatedProductName"/></regulatedProductName>	
							      		 		<errorMessage>Regulated Product Name (#regulatedProductName#) must be provided for all consumer units (isTradeItemAConsumerUnit is equal to 'true') from this Global Product Category Code (#gpcCategoryCode#).</errorMessage>
