...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
   ...
   <pattern>
      ...
      <doc:description>If targetMarketCountryCode equals ('008' (Albania), '051' (Armenia), '031' (Azerbaijan), '040' (Austria), '112' (Belarus), '056' (Belgium), '070' (Bosnia-Herzegovina), '100' (Bulgaria), 
                                                          ...
                                                          and dailyValueIntakePercent is used 
                                                          and (nutrientTypeCode is used with one of the values: 'FAT', 'FASAT', 'ENER-', 'CHOAVL', 'PRO-', 'SUGAR-' or 'SALTEQ'),
                                                          then dailyValueIntakeReference SHALL be used.	
      </doc:description>    
      ...
      <doc:attribute2>dailyValueIntakeReference</doc:attribute2>       
   	  <rule context="nutrientHeader">
		    <assert test="if((ancestor::tradeItem/targetMarket/targetMarketCountryCode =  ('008' , '051' , '031' , '040', '112' , '056' , '070' , '100' , '191' , '196' , '203' , '233' , '246' , '250' , '268' , '300' , '348' ,
        				    ...
        				    and (exists(nutrientDetail/dailyValueIntakePercent))
        					...
        					and ( (nutrientDetail/nutrientTypeCode = 'FAT')
        					or (nutrientDetail/nutrientTypeCode = 'FASAT') 
        					or (nutrientDetail/nutrientTypeCode = 'ENER-')                             
        					or (nutrientDetail/nutrientTypeCode = 'CHOAVL') 
        					or (nutrientDetail/nutrientTypeCode = 'PRO-')         					
                            or (nutrientDetail/nutrientTypeCode = 'SUGAR-') 
        					or (nutrientDetail/nutrientTypeCode = 'SALTEQ')         					     
        					))         					    					
        					then (exists(dailyValueIntakeReference)
        					and  (every $node in (dailyValueIntakeReference) 
        					satisfies ($node != '')))                    												
        							
				          				 else true()">					          		 
				          		 		
							      		 		<errorMessage>Daily value intake reference shall be filled out in case the nutrient type code equals 'FAT', 'FASAT', 'ENER-', 'CHOAVL', 'PRO-', 'SUGAR-' or 'SALTEQ' and daily value intake percent is filled out for the target market.</errorMessage>
