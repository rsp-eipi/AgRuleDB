...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
   ...
   <pattern>
      ...
      <doc:description>If targetMarketCountryCode equals ('008' (Albania), '051' (Armenia), '031' (Azerbaijan), '040' (Austria), '112' (Belarus), '056' (Belgium), '070' (Bosnia-Herzegovina), '100' (Bulgaria), '191' (Croatia),
                                                          ...
                                                          '643' (Russia), '688' (Serbia), '703' (Slovakia), '705' (Slovenia), '752' (Sweden), '756' (Switzerland), '792' (Turkey), '795' (Turkmenistan),
                                                          '826' (United Kingdom), '804' (Ukraine), or '860' (Uzbekistan))
                                                           and one instance of nutrientTypeCode equals 'STARCH',
      ...
      <doc:attribute2>nutrientTypeCode</doc:attribute2>       
   	  <rule context="nutrientHeader">
		    <assert test="if((ancestor::tradeItem/targetMarket/targetMarketCountryCode =  ('008' , '051' , '031' , '040', '112' , '056' , '070' , '100' , '191' , '196' , '203' , '233' , '246' , '250' , '268' , '300' , '348' ,
        					...
        					and (nutrientDetail/nutrientTypeCode = 'STARCH'))        					      					    					
        					then (some $node in (nutrientDetail) 
        					satisfies ($node/nutrientTypeCode = 'CHOAVL'))                    												
        							
				          				 else true()">					          		 
				          		 		
							      		 		<errorMessage>If nutrient type code is filled out with 'STARCH', then there shall be another nutrient type code with value 'CHOAVL' within the same group, for the target market.</errorMessage>
