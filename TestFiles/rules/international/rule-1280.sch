...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
    ...
    <pattern>
        <title>Rule 1280</title>
        <doc:description>If tradeItemUnitDescriptorCode is equal to 'DISPLAY_SHIPPER' then child item tradeItemUnitDescriptorCode must not equal 'TRANSPORT_LOAD', 'MIXED_MODULE' or ' PALLET'.</doc:description>
        <doc:attribute1>TradeItem/TradeItemUnitDescriptorCode</doc:attribute1>
        <rule context="catalogueItem">
            <assert test="if (tradeItem/tradeItemUnitDescriptorCode= 'DISPLAY_SHIPPER')
                          then (every $node in (catalogueItemChildItemLink/catalogueItem/tradeItem/tradeItemUnitDescriptorCode) 
                          satisfies(($node != 'TRANSPORT_LOAD') and ($node != 'MIXED_MODULE')
                          and ($node != 'PALLET')))
                          
                           else true()">
                           
                 					<errorMessage>If tradeItemUnitDescriptorCode is equal to 'DISPLAY_SHIPPER' 
                 					              then child item tradeItemUnitDescriptorCode must not equal 'TRANSPORT_LOAD', 'MIXED_MODULE' or ' PALLET'.
 					                </errorMessage> 
