...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
   ...
   <pattern>
      <title>Rule 1688</title>
      <doc:description>If targetMarketCountryCode equals ‘250’ (France) and gpcCategoryCode is in GPC Family ('67010000' (Clothing) or ‘63010000’ (Footwear)) and tradeItemUnitDescriptorCode equals ‘BASE_UNIT_OR_EACH’ and isTradeItemAConsumerUnit equals ‘true’ then one iteration of colourCode shall be provided.</doc:description>
      ...
      <rule context="tradeItem">
          <assert test="if((targetMarket/targetMarketCountryCode =  ('250'(: France :) )) 
    					...
    					and (some $node in (tradeItemInformation/extension/*:tradeItemDescriptionModule/tradeItemDescriptionInformation/colour/colourCode)
    					satisfies ($node != '')))   
    					
    						 else true()">			        		 
				          		 		
							      		 		<errorMessage>If the target market is France, the product has a GPC brick code belonging to the family ('67010000' (clothing) or'63010000' (footwear)), and the hierarchy level is a base unit that is also a consumer unit then at least one colour code must be provided.</errorMessage>
