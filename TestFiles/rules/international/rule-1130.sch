   ...
   <pattern>
      ...
      <rule context="tradeItem">
		  <assert test="if((targetMarket/targetMarketCountryCode =  ('249'(: France :) , '250'(: France :) )) 
    					...
    					and (tradeItemInformation/extension/*:packagingInformationModule/packaging/platformTypeCode = '40'))
    					...
    					then   ((number($node/depth) ge 1016) and (number($node/depth) le 2032)
            			and    (number($node/width) ge 1219) and (number($node/width) le 2438))                			
            			else  (: measurementUnitCode = 'IN' :)    
            			     if (($node/depth/@measurementUnitCode = 'IN') and  ($node/width/@measurementUnitCode = 'IN'))              			    
            			then   ((number($node/depth) ge 40) and (number($node/depth) le 80)
            			and    (number($node/width) ge 47.99) and (number($node/width) le 95.98))                   			 
            				else true()  
            					else true())) 												
        							
				          				 else true()">				          		 
				          		 		
							      		 		<errorMessage>If targetMarketCountryCode equals ('249' (France) or '250' (France)) and tradeItemUnitDescriptorCode equals 'PALLET' and platformTypeCode equals  '40',
							      		 		              then TradeItemMeasurements/depth shall be between and including ('1016 MMT' or '40 IN') and ('2032 MMT' or '80 IN')
							      		 		              and TradeItemMeasurements/width shall be between and including ('1219 MMT' or '47.99 IN') and ('2438 MMT' or '95.98 IN').
	                                            </errorMessage>
