   ...
   <pattern>
      ...
      <doc:description>If targetMarketCountryCode equals ('008' (Albania), '051' (Armenia), '031' (Azerbaijan), '040' (Austria), '112' (Belarus), '056' (Belgium), '070' (Bosnia-Herzegovina), '100' (Bulgaria), 
                                                          ...
                                                          '795' (Turkmenistan), '826' (United Kingdom), '804' (Ukraine), or '860' (Uzbekistan))
                                                           and (nutrientBasisQuantity equals 100 and nutrientBasisQuantity /@measurementUnitCode equals 'GRM')
                                                           and (nutrientTypeCode equals 'FAT' and quantityContained/UoM is used with 'GRM')
                                                           and (nutrientTypeCode is used with 'PRO-' and quantityContained/@measurementrUnitCode equals 'GRM')
                                                           and (nutrientTypeCode equals 'CHOAVL' and quantityContained/@measuremenrUnitCode equals 'GRM'), 
                                                           then the sum of the corresponding quantityContained values SHALL be less than 102 gram per nutrientHeader.
      </doc:description>    
      <doc:attribute1>targetMarketCountryCode,gpcCategoryCode,nutrientTypeCode,</doc:attribute1> 
      <doc:attribute2>nutrientBasisQuantity, nutrientTypeCode, quantityContained</doc:attribute2>       
   	   <rule context="nutrientHeader">
		    <assert test="if((ancestor::tradeItem/targetMarket/targetMarketCountryCode =  ('008' , '051' , '031' , '040', '112' , '056' , '070' , '100' , '191' , '196' , '203' , '233' , '246' , '250' , '268' , '300' , '348' ,
        				    ...
        				    and (nutrientBasisQuantity = 100)
        				    and (nutrientBasisQuantity/@measurementUnitCode = 'GRM')
        				    ) 
        				    then (every $node in (.[exists(./nutrientDetail/nutrientTypeCode[.='FAT'])
		           		    and exists(./nutrientDetail/nutrientTypeCode[.='PRO-'])     
		           		    and exists(./nutrientDetail/nutrientTypeCode[.='CHOAVL'])]) 
		           		    satisfies (every $node1 in ($node/nutrientDetail[nutrientTypeCode[.='FAT']]/quantityContained)
		           		    satisfies (every $node2 in ($node/nutrientDetail[nutrientTypeCode[.='PRO-']]/quantityContained)
		            	    satisfies (every $node3 in ($node/nutrientDetail[nutrientTypeCode[.='CHOAVL']]/quantityContained)
		               	    satisfies ( (number($node1) + number($node2) + number($node3)  ) &lt; 102)  ))))         					          												
        							
				          				 else true()">	
				          		 		
							      		 		<errorMessage>The sum of the quantity contained values for nutrient type codes 'FAT', 'PRO-' and 'CHOAVL' exceeds 100 gram.</errorMessage>
