...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" 
   ...
   <pattern>
      ...
      <doc:description>If targetMarketCountryCode equals ('056' (: Belgium :), '442' (: Luxembourg :), '528' (: Netherlands :), '276' (: Germany :), '208' (: Denmark :), 
                       ...
                       and one instance of nutrientTypeCode equals 'FAPUCIS' and quantityContained is used 
                       and another instance of nutrientTypeCode equals 'FAT' and quantityContained is used, 
                       then quantityContained for nutrientTypeCode 'FAPUCIS' SHALL be less than or equal to quantityContained for nutrientTypeCode 'FAT'.
      </doc:description>
      ...
      <rule context="tradeItem">
		  <assert test="if (targetMarket/targetMarketCountryCode =  ('056', '442', '528', '276', '208', '203', '246', '826', '380', '250', '040'))
		                then (every $node in (tradeItemInformation/extension/*:nutritionalInformationModule/nutrientHeader[exists(./nutrientDetail/nutrientTypeCode[.='FAPUCIS'])
		                and exists(./nutrientDetail/nutrientTypeCode[.='FAT'])]) 
		                satisfies(every $node1 in ($node/nutrientDetail[nutrientTypeCode[.='FAPUCIS']]/quantityContained)
		                satisfies (every $node2 in ($node/nutrientDetail[nutrientTypeCode[.='FAT']]/quantityContained) satisfies (number($node1) &lt;= number($node2))))) 
           							
        							
				          				 else true()">				          		 
				          		 		
							      		 		<errorMessage>Quantity of total polyunsaturated-cis fatty acids ('FAPUCIS') is greater than the quantity of total fat ('FAT').</errorMessage>
