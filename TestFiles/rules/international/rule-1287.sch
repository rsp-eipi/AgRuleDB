...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
   ...
   <pattern>
      <title>Rule 1287</title>
       <doc:description>There must be at most one iteration of maximumToleranceTemperature per value of temperatureQualifierCode</doc:description>
       <doc:attribute1>TradeItemTemperatureInformation/temperatureQualifierCode</doc:attribute1>
       <doc:attribute2>TradeItemTemperatureInformation/maximumToleranceTemperature</doc:attribute2>
       <rule context="tradeItem/tradeItemInformation/extension/*:tradeItemTemperatureInformationModule">
           <assert test="every $node in ('DELIVERY_TO_DISTRIBUTION_CENTRE','DELIVERY_TO_MARKET','INLET_TEMPERATURE','OPERATING_TEMPERATURE','STORAGE_AFTER_OPENING','STORAGE_AFTER_OVERWRAP','STORAGE_AFTER_RECONSTITUTION','STORAGE_HANDLING','TRANSPORTATION','WORKING_TEMPERATURE')
           				 ...
           				 then (count(tradeItemTemperatureInformation[temperatureQualifierCode=$node]/maximumToleranceTemperature) &lt;= 1)
                         
                         	 else true())">                         	 
                         	 
                         	 		<errorMessage>There must be at most one iteration of maximumToleranceTemperature per value of temperatureQualifierCode</errorMessage> 
