   ...
   <pattern>
      ...
      <rule context="tradeItem">
		  <assert test="if((targetMarket/targetMarketCountryCode =  ('249'(: France :) , '250'(: France :) )) 
    					...
    					and (tradeItemInformation/extension/*:packagingInformationModule/packaging/platformTypeCode = '31'))
    					...
    					then   ((number($node/depth) ge 1000) and (number($node/depth) le 2000)
            			and    (number($node/width) ge 600) and (number($node/width) le 1200))                			
            			else  (: measurementUnitCode = 'IN' :)    
            			     if (($node/depth/@measurementUnitCode = 'IN') and  ($node/width/@measurementUnitCode = 'IN'))              			    
            			then   ((number($node/depth) ge 39.37) and (number($node/depth) le 78.74)
            			and    (number($node/width) ge 23.62) and (number($node/width) le 47.34))                   			 
            				else true()  
            					else true())) 												
        							
				          				 else true()">				          		 
				          		 		
							      		 		<errorMessage>If targetMarketCountryCode equals ('249' (France) or '250' (France)) and tradeItemUnitDescriptorCode equals 'PALLET' and platformTypeCode equals  '31',
							      		 		              then TradeItemMeasurements/depth shall be between and including  ('1000 MMT' or '39.37 IN') and ('2000 MMT' or '78.74 IN')
							      		 		              and TradeItemMeasurements/width shall be between and including ('600 MMT' or '23.62 IN') and ('1200 MMT' or '47.34 IN').
	                                            </errorMessage>
