...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
   ...
   <pattern>
      <title>Rule 1333</title>
       <doc:description>If cancelDateTime is not empty then discontinuedDateTime must be empty.</doc:description>
       <doc:attribute1>CatalogueItemDates/cancelDateTime</doc:attribute1>
       <doc:attribute2>CatalogueItemDates/discontinuedDateTime</doc:attribute2>
       <rule context="catalogueItemDates">
           <assert test="if (cancelDateTime) then (every $node in (discontinuedDateTime) satisfies (empty($node)))
